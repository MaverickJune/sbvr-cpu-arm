...
//         //--------------------------------------------------
//         // k-loop : AND-VCNT → FP16 변환 → 즉시 FMLAL
//         //--------------------------------------------------
                  ...
                  int K)      
{
    ...
    for(int bvr_idx = 0; bvr_idx < bvr_per_K; ++bvr_idx)
    {
        ...
        {
            __fp16      a0   = l_coeff_cache[l_coeff_i * LNumSums + l];
            float16x8_t a0v  = vdupq_n_f16(a0);

            __fp16      a1   = l_coeff_cache[l_coeff_i * LNumSums + l + 1];
            ...
            for (int r = 0; r < RNumSums; ++r)
            {
                ...
                float16x8_t ab0 = vmulq_f16(a0v, b0);   // lanes 0‥7
                float16x8_t ab1 = vmulq_f16(a0v, b1);   // lanes 8‥15
                ...
                uint8x16_t pc_u8_1 = popc_cache[l + 1][r];
                float16x8_t pc0_1  = vcvtq_f16_u16(vmovl_u8(vget_low_u8 (pc_u8_1)));
                float16x8_t pc1_1  = vcvtq_f16_u16(vmovl_u8(vget_high_u8(pc_u8_1)));

                float16x8_t ab0_1 = vmulq_f16(a1v, b0);
                float16x8_t ab1_1 = vmulq_f16(a1v, b1);

                acc0 = vfmlalq_low_f16 (acc0, ab0_1, pc0_1);
                acc1 = vfmlalq_high_f16(acc1, ab0_1, pc0_1);
                acc2 = vfmlalq_low_f16 (acc2, ab1_1, pc1_1);
                acc3 = vfmlalq_high_f16(acc3, ab1_1, pc1_1);
...
//         coeffs<LNumSums> l_coeffs = *(coeffs<LNumSums>*)(&l_coeff_cache[l_coeff_i * LNumSums]);
