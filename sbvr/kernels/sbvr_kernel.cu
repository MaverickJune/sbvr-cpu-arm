...
__global__ void sbvr_per_elem_mm(uint32_t* l_bvr,
                                 uint8_t* l_coeff_idx,
                                 uint8_t* l_bias_idx,
                                 ...
                                 int num_cgroups) 
{
    ...
    #define IDX(i, j) ((i) * (r_num_sums + 1) + (j))
    __half* coeff_mult = smem;
    ...
    for (int i = tid; i < total_outputs; i += blockDim.x * gridDim.x) 
    {
        ...
        {
            int l_coeff_cache_idx = l_coeff_idx[row * cgroup_per_inner_vec + cg_idx];
            int l_bias_cache_idx = l_bias_idx[row * cgroup_per_inner_vec + cg_idx];
            __half* l_coeff_cache = &l_coeff_cache[l_coeff_cache_idx * l_num_sums];
            __half l_bias = l_bias_cache[l_bias_cache_idx];
            int r_coeff_cache_idx = r_coeff_idx[col * cgroup_per_inner_vec + cg_idx];
            int r_bias_cache_idx = r_bias_idx[col * cgroup_per_inner_vec + cg_idx];
            __half* r_coeff_cache = &r_coeff_cache[r_coeff_cache_idx * r_num_sums];
            ...
            {
                coeff_mult[0][r_idx + 1] = __hmul(l_bias, r_coeff_cache[r_idx]);
            }
            for (int l_idx = 0; l_idx < l_num_sums; l_idx++)
            {
                __half l_coeff = l_coeff_cache[l_idx];
                coeff_mult[l_idx + 1][0] = __hmul(l_coeff, r_bias);
                for (int r_idx = 0; r_idx < r_num_sums; r_idx++)
                {
                    coeff_mult[l_idx + 1][r_idx + 1] = 
                        __hmul(l_coeff, r_coeff_cache[r_idx]);
                }
            }
            ...
            for (int bvr_idx = 0; bvr_idx < bvr_per_cgroup; bvr_idx++)
            {
                ...
                for (int r_idx = 0; r_idx < r_num_sums; r_idx++)
                {
                    // r_bvr: [out_cols, self.num_sums, cgroup_per_inner_vec, bvr_per_cgroup]
                    uint32_t r = r_bvr[
                                 col * cgroup_per_inner_vec * bvr_per_cgroup * r_num_sums +
                ...
                for (int l_idx = 0; l_idx < l_num_sums; l_idx++)
                {
                    // l_bvr: [out_rows, self.num_sums, cgroup_per_inner_vec, bvr_per_cgroup]
                    uint32_t l = l_bvr[
                                 row * cgroup_per_inner_vec * bvr_per_cgroup * l_num_sums +
                    ...
                    for (int r_idx = 0; r_idx < r_num_sums; r_idx++)
                    {
                        ...
                        sum += lr_popc * __half2float(coeff_mult[l_idx + 1][r_idx + 1]);
                    }
                }
            }
                              ...
                              int num_cgroups) 
{
    ...
    __half coeff_mult[l_num_sums + 1][r_num_sums + 1];
